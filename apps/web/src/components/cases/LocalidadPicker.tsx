...
export default function LocalidadPicker({ onPick }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40">
      <div className="absolute left-0 right-0 top-0 sm:top-1/2 sm:left-1/2 sm:right-auto sm:-translate-x-1/2 sm:-translate-y-1/2 sm:w-full sm:max-w-sm bg-white dark:bg-gray-800 shadow-xl rounded-b-2xl sm:rounded-2xl p-5 flex flex-col gap-4">
        <div>
          <h2 className="text-base font-semibold text-gray-900 dark:text-gray-100">¿En qué zona buscás?</h2>
        ...
        <div className="relative">
          <input
            ...
            className="w-full px-3 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg text-base focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent"
          />
          {searching && (
        ...
        {geoError && (
          <p className="text-xs text-red-600 dark:text-red-300 -mt-2">{geoError}</p>
        )}
      </div>
    </div>
  )

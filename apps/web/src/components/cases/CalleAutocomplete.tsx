  ...
  return (
    <div ref={containerRef} className="relative flex flex-col gap-1">
      ...
      {open && suggestions.length > 0 && (
        <ul className="absolute z-50 left-0 right-0 top-full mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg max-h-48 overflow-y-auto">
          {suggestions.map((s, i) => {
            const name = s.name || s.display_name.split(',')[0].trim()
            return (
              <li key={i}>
